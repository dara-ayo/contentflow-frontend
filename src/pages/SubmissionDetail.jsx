...
// --- Image Recommendations Component ---
function ImageRecommendations({ content, platform }) {
  const [showImages, setShowImages] = useState(false)

  if (!content || content.length < 50) return null

  // Extract key topics from the content for image search suggestions
  const stopWords = new Set(['the','a','an','and','or','but','in','on','at','to','for','of','with','is','it','this','that','are','was','were','be','been','have','has','do','does','will','would','could','should','may','might','can','by','from','not','your','their','our','you','we','they','he','she','its','my','me','us','so','if','how','what','when','where','who','why','which','more','most','also','just','than','then','about','into','over','after','before','between','under','through','during','each','every','all','both','few','some','any','other','new','old','many','much','very','only','even','still','already','now','here','there','where'])

  const words = content.toLowerCase().replace(/<[^>]+>/g, ' ').replace(/[^a-z\s]/g, ' ').split(/\s+/).filter(w => w.length > 3 && !stopWords.has(w))
  const freq = {}
  words.forEach(w => { freq[w] = (freq[w] || 0) + 1 })
  const topWords = Object.entries(freq).sort((a, b) => b[1] - a[1]).slice(0, 5).map(([w]) => w)

  const platformSizes = {
    linkedin: { label: '1200 × 627px (landscape)', aspect: 'LinkedIn post' },
    twitter: { label: '1600 × 900px (16:9)', aspect: 'X/Twitter post' },
    newsletter: { label: '600 × 400px (email-safe)', aspect: 'Email header' },
  }

  const searchTerms = [
    topWords.slice(0, 3).join(' '),
    topWords.slice(0, 2).join(' ') + ' business',
    topWords[0] + ' technology modern',
  ]
  ...
  return (
    <div className="mt-4">
      ...
      >
        <svg className={`w-3.5 h-3.5 transition-transform duration-200 ${showImages ? 'rotate-90' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {showImages ? 'Hide image suggestions' : 'Show image suggestions'}
      </button>

      {showImages && (
        <div className="mt-3 p-4 rounded-xl bg-surface-tertiary border border-surface-border animate-fade-in">
          <div className="flex items-center gap-2 mb-3">
            <svg className="w-4 h-4 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
            </svg>
            <span className="text-xs font-medium text-text-primary">Image Recommendations for {size.aspect}</span>
          </div>

          <p className="text-[11px] text-text-tertiary mb-3">
            Recommended size: <span className="font-mono text-text-secondary">{size.label}</span>
          </p>

          <div className="space-y-2 mb-4">
            <p className="text-[11px] font-medium text-text-tertiary uppercase tracking-wider">Search terms for free stock images:</p>
            {searchTerms.map((term, i) => (
              <div key={i} className="flex items-center gap-2">
                <span className="text-xs text-text-secondary font-mono bg-surface-secondary px-2.5 py-1 rounded-lg border border-surface-border">
                  {term}
                </span>
                <a
                ...
                <a
                  href={`https://www.pexels.com/search/${encodeURIComponent(term)}/`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-[11px] text-accent hover:text-accent-hover transition-colors"
                >
          ...
          <div className="pt-3 border-t border-surface-border">
            <p className="text-[11px] text-text-tertiary">
              <span className="font-medium text-text-secondary">Tip:</span> Use high-contrast images with minimal text overlay. For {platform === 'linkedin' ? 'LinkedIn, professional headshots and workplace imagery perform best' : platform === 'twitter' ? 'X/Twitter, eye-catching visuals and data graphics drive engagement' : 'newsletters, clean header images that match your brand colors work best'}.
            </p>
          </div>
        </div>
      )}
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {showAdaptedContent && (
        <div className="space-y-0">
          ...
          <ImageRecommendations content={activeTab === 'linkedin' ? editLinkedin : activeTab === 'twitter' ? editTwitter : editNewsletterContent} platform={activeTab} />
